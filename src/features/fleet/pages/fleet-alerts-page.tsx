...
const FLEET_KINDS: NotificationKind[] = ['vehicle_in_maintenance', 'trip_in_progress_long']
...
export function FleetAlertsPage() {
  ...
  const fleetAlerts = useMemo(
    () => notifications.filter((n) => FLEET_KINDS.includes(n.kind)),
    [notifications],
  )

  const filtered = useMemo(() => {
    return fleetAlerts.filter((n) => {
  ...
  }, [fleetAlerts, severity, unreadOnly, search])

  const fleetUnread = fleetAlerts.filter((n) => !n.read).length

  return (
    <div>
      <PageHeader
        title="Alerts"
        subtitle={`${fleetAlerts.length} alert${fleetAlerts.length === 1 ? '' : 's'}${fleetUnread > 0 ? ` · ${fleetUnread} unread` : ''}`}
        actions={
          fleetUnread > 0 && (
            <Button
              variant="outline"
          ...
          <div className="px-6 py-16 text-center">
            {fleetAlerts.length === 0 ? (
              <>
                ...
                <p className="text-[12.5px] text-zinc-500 mt-1">No vehicles in maintenance and no long-running trips.</p>
              </>
            ) : (
              <>
