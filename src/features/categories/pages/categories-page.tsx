...
export function CategoriesPage() {
  ...
  return (
    <div>
      ...
      <Modal open={modalMode !== 'closed'} onClose={closeModal} title={isEditing ? 'Edit Category' : 'Add Category'} size="md">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" fullWidth loading={submitting}>{isEditing ? 'Save Changes' : 'Add Category'}</Button>
          </div>
        </form>
      </Modal>

      <Modal open={!!deleteCandidate} onClose={() => setDeleteCandidate(null)} title="Delete Category" size="sm">
        {deleteCandidate && (
