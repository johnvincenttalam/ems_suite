...
export function InventoryAlertsPage() {
  ...
  const invAlerts = useMemo(
    () => notifications.filter((n) => INV_KINDS.includes(n.kind)),
    [notifications],
  )

  const filtered = useMemo(() => {
    return invAlerts.filter((n) => {
  ...
  }, [invAlerts, severity, unreadOnly, search])

  const invUnread = invAlerts.filter((n) => !n.read).length

  return (
    <div>
      <PageHeader
        title="Alerts"
        subtitle={`${invAlerts.length} alert${invAlerts.length === 1 ? '' : 's'}${invUnread > 0 ? ` · ${invUnread} unread` : ''}`}
        actions={
          invUnread > 0 && (
            <Button
              variant="outline"
          ...
          <div className="px-6 py-16 text-center">
            {invAlerts.length === 0 ? (
              <>
                ...
                <p className="text-[14px] font-medium text-zinc-700">All stocked</p>
                <p className="text-[12.5px] text-zinc-500 mt-1">No low-stock or stock-out items right now.</p>
              </>
            ) : (
              <>
