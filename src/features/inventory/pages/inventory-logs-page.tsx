...
function InventoryLogsPageInner() {
  ...
  const invEntries = useMemo(
    () => allEntries.filter((e) => e.module === 'Inventory'),
    [allEntries],
  )

  const filtered = useMemo(
    () => actionFilter === 'all' ? invEntries : invEntries.filter((e) => e.action === actionFilter),
    [invEntries, actionFilter],
  )
  ...
  return (
    <div>
      <PageHeader
        title="System Logs"
        subtitle={`${invEntries.length} inventory event${invEntries.length === 1 ? '' : 's'} recorded`}
        actions={
          <ExportMenu
            rows={invEntries as unknown as Record<string, unknown>[]}
            baseFilename="inventory-logs"
            sheetName="Inventory Logs"
            pdfTitle="Inventory System Logs"
            pdfSubtitle={`${invEntries.length} event${invEntries.length === 1 ? '' : 's'} recorded`}
            columns={[
