...
export function AuditLogPage() {
  ...
  const columns = useMemo<ColumnDef<AuditEntry>[]>(() => [
    ...
    { accessorKey: 'module', header: 'Module', cell: ({ getValue }) => <span className="text-zinc-700">{getValue() as string}</span> },
    ...
    <div>
      <PageHeader
        ...
        actions={
          <ExportMenu
            ...
            sheetName="Audit Log"
            pdfTitle="System Audit Log"
            pdfSubtitle={`${entries.length} event${entries.length === 1 ? '' : 's'} recorded`}
            columns={[
      ...
      <div className="mb-4 flex flex-col sm:flex-row gap-3 sm:items-center">
        <div className="max-w-sm flex-1">
          <SearchInput value={globalFilter} onChange={setGlobalFilter} placeholder="Search by user, module, or detail..." />
        </div>
