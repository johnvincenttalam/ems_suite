...
export function AssetsUsersPage() {
  ...
  const revokeMutation = useMutation({
    mutationFn: (userId: string) => {
      ...
      return usersApi.setModuleRole({ userId, moduleKey: 'assets', role: null, auditModule: 'Assets', byId: currentUser.id })
    },
    onSuccess: (user) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {isModuleAdmin(row.original, 'assets') && (
              ...
              <ModuleAccessPills modules={userModules(row.original)} excludeModule="assets" className="mt-1" />
            </div>
          </div>
        ),
      },
    ...
    if (canManage) {
      base.push({
        ...
        cell: ({ row }) => {
          ...
          const isAdmin = isModuleAdmin(u, 'assets')
  ...
  return (
    <div>
      ...
      <ListToolbar
        search={{ value: globalFilter, onChange: setGlobalFilter, placeholder: 'Search asset users...' }}
      />
      ...
      <DataTable
        ...
        columns={columns}
        emptyIcon={Users}
        emptyMessage="No asset users found"
        onRowClick={(user) => navigate(getModulePath('assets', `users/${user.id}`))}
      />
