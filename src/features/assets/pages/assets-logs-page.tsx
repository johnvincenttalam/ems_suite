...
export function AssetsLogsPage() {
  return (
    <ModuleAdminGuard moduleKey="assets" pageLabel="Assets Logs">
      <AssetsLogsPageInner />
    </ModuleAdminGuard>
  )
}

function AssetsLogsPageInner() {
  const { data: allEntries = [], isLoading } = useAuditLog()
  const [globalFilter, setGlobalFilter] = useState('')
  const [actionFilter, setActionFilter] = useState<AuditAction | 'all'>('all')

  const assetEntries = useMemo(
    () => allEntries.filter((e) => e.module === 'Assets'),
    [allEntries],
  )

  const filtered = useMemo(
    () => actionFilter === 'all' ? assetEntries : assetEntries.filter((e) => e.action === actionFilter),
    [assetEntries, actionFilter],
  )
  ...
  return (
    <div>
      <PageHeader
        title="System Logs"
        subtitle={`${assetEntries.length} asset event${assetEntries.length === 1 ? '' : 's'} recorded`}
        actions={
          <ExportMenu
            rows={assetEntries as unknown as Record<string, unknown>[]}
            baseFilename="assets-logs"
            sheetName="Assets Logs"
            pdfTitle="Assets System Logs"
            pdfSubtitle={`${assetEntries.length} event${assetEntries.length === 1 ? '' : 's'} recorded`}
            columns={[
      ...
      <ListToolbar
        ...
        filter={
          <FilterChips
            ...
            value={actionFilter}
            onChange={(v) => setActionFilter(v as AuditAction | 'all')}
          />
        }
      />
