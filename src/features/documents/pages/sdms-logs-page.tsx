...
export function SdmsLogsPage() {
  return (
    <ModuleAdminGuard moduleKey="sdms" pageLabel="SDMS Logs">
      <SdmsLogsPageInner />
    </ModuleAdminGuard>
  )
}

function SdmsLogsPageInner() {
  const { data: allEntries = [], isLoading } = useAuditLog()
  const [globalFilter, setGlobalFilter] = useState('')
  const [actionFilter, setActionFilter] = useState<AuditAction | 'all'>('all')

  const sdmsEntries = useMemo(
    () => allEntries.filter((e) => e.module === 'Documents'),
    [allEntries],
  )

  const filtered = useMemo(
    () => actionFilter === 'all' ? sdmsEntries : sdmsEntries.filter((e) => e.action === actionFilter),
    [sdmsEntries, actionFilter],
  )
  ...
  return (
    <div>
      <PageHeader
        title="System Logs"
        subtitle={`${sdmsEntries.length} document event${sdmsEntries.length === 1 ? '' : 's'} recorded`}
      />
      ...
      >
        <ExportMenu
          rows={sdmsEntries as unknown as Record<string, unknown>[]}
          baseFilename="sdms-logs"
          sheetName="SDMS Logs"
          pdfTitle="SDMS System Logs"
          pdfSubtitle={`${sdmsEntries.length} event${sdmsEntries.length === 1 ? '' : 's'} recorded`}
          columns={[
