...
const MAINT_KINDS: NotificationKind[] = ['wo_assigned', 'wo_due_soon', 'wo_overdue']
...
export function MaintenanceAlertsPage() {
  ...
  const maintAlerts = useMemo(
    () => notifications.filter((n) => MAINT_KINDS.includes(n.kind)),
    [notifications],
  )

  const filtered = useMemo(() => {
    return maintAlerts.filter((n) => {
  ...
  }, [maintAlerts, severity, unreadOnly, search])

  const maintUnread = maintAlerts.filter((n) => !n.read).length

  return (
    <div>
      <PageHeader
        title="Alerts"
        subtitle={`${maintAlerts.length} alert${maintAlerts.length === 1 ? '' : 's'}${maintUnread > 0 ? ` · ${maintUnread} unread` : ''}`}
        actions={
          maintUnread > 0 && (
            <Button
              variant="outline"
          ...
          <div className="px-6 py-16 text-center">
            {maintAlerts.length === 0 ? (
              <>
                ...
                <p className="text-[14px] font-medium text-zinc-700">All clear</p>
                <p className="text-[12.5px] text-zinc-500 mt-1">No work orders due soon, overdue, or newly assigned to you.</p>
              </>
            ) : (
              <>
