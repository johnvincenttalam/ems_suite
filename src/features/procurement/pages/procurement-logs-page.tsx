...
export function ProcurementLogsPage() {
  return (
    <ModuleAdminGuard moduleKey="procurement" pageLabel="Procurement Logs">
      <ProcurementLogsPageInner />
    </ModuleAdminGuard>
  )
}

function ProcurementLogsPageInner() {
  const { data: allEntries = [], isLoading } = useAuditLog()
  const [globalFilter, setGlobalFilter] = useState('')
  const [actionFilter, setActionFilter] = useState<AuditAction | 'all'>('all')

  const procEntries = useMemo(
    () => allEntries.filter((e) => e.module === 'Procurement'),
    [allEntries],
  )

  const filtered = useMemo(
    () => actionFilter === 'all' ? procEntries : procEntries.filter((e) => e.action === actionFilter),
    [procEntries, actionFilter],
  )
  ...
  return (
    <div>
      <PageHeader
        title="System Logs"
        subtitle={`${procEntries.length} procurement event${procEntries.length === 1 ? '' : 's'} recorded`}
      />
      ...
      >
        <ExportMenu
          rows={procEntries as unknown as Record<string, unknown>[]}
          baseFilename="procurement-logs"
          sheetName="Procurement Logs"
          pdfTitle="Procurement System Logs"
          pdfSubtitle={`${procEntries.length} event${procEntries.length === 1 ? '' : 's'} recorded`}
          columns={[
