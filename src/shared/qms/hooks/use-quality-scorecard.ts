...
export function useQualityScorecard(): UseQualityScorecardResult {
  ...
  const isLoading = woLoading || reqLoading || docLoading || invLoading || vehicleLoading

  const scorecard = useMemo(
    () => deriveQualityScorecard({ workOrders, requests, documents, items, vehicles }),
    [workOrders, requests, documents, items, vehicles],
  )
